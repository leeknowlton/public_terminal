...
export async function GET() {
  ...
  const messageCount = await client.readContract({
  ...
  // Fetch the most recent 4 messages
  const latestId = BigInt(total);
  const ids = [latestId - 3n, latestId - 2n, latestId - 1n, latestId].filter(id => id >= 1n);
